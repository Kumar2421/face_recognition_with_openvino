...
faceIDENTIFY = "models\\face-reidentification-retail-0095.xml"
...
class FrameProcessor:
    ...
    def _init_(self,fqa_model_path):
        ...
        self.faces_database = FacesDatabase("employees", self.face_identifier, self.landmarks_detector)
...
def draw_face_detection(frame, frame_processor, detections, camera_id, fqa_threshold):
    ...
    for roi, landmarks, identity in detections:
        ...
        # Find matched object ID based on centroid
        matched_object_id = find_matched_object_id(centroid)
        
        # if fqa_score >= fqa_threshold:
        image_recognizer(frame, text, identity, (xmin, ymin, xmax, ymax), 0.75, camera_id, matched_object_id)  
    return frame
...
def image_recognizer(frame, text, identity, face_rect, threshold, camera_id, matched_object_id):
    ...
    if identity.id != FaceIdentifier.UNKNOWN_ID:
        if (1 - identity.distance) > threshold:
            ...
            image_path = capture_and_save_image(frame, f"{matched_object_id}{text}{datetime.now().strftime('%Y-%m-%d_%H-%M-%S')}", (xmin, ymin, xmax - xmin, ymax - ymin))  # Update image_path
            # try:
            #     if camera_id == 'A':
            ...
            image_path = capture_and_save_image(frame, f"{matched_object_id}UNKNOWN{datetime.now().strftime('%Y-%m-%d_%H-%M-%S')}", (xmin, ymin, xmax - xmin, ymax - ymin))
...
frame_processor_1 = FrameProcessor("models\\face-quality-assessment.onnx")
...
line_position = 250  # Adjust the position as needed
...
max_consecutive_frames_lost = 10
